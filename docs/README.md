# Docs Guide

작성일: 2026-04-06
상태: 현재 참조 문서만 남기고 나머지는 아카이브로 정리한 기준 문서

## 목적

`docs` 디렉터리는 이제 현재 시점에서 바로 참고해야 하는 문서만 앞에 두고, 이전 리뷰 기록과 작업 로그는 아카이브로 분리한다.
## 현재 참고 순서

1. `docs/plans/2026-04-21-flippers-master-plan.md`
2. `docs/Current/Flippers-Current-Priorities-2026-04-06.md`
3. `docs/Current/Flippers-Current-Status-2026-04-06.md`
4. `docs/Current/Flippers-Release-Privacy-Summary-2026-04-06.md`
5. `docs/operations/2026-04-21-flippers-ocr-proxy-runbook.md`
6. `SRS.md`
7. 필요 시 `CLAUDE.md`

## 아카이브 사용 규칙

- 현재 참고 문서만으로 해결되지 않는 부분이 있을 때만 아카이브 문서를 확인한다.
- 과거 판단 근거, 세부 구현 로그, 이전 리뷰 문구, 긴 초안 원문이 필요할 때만 아카이브를 연다.
- 현재 작업 판단은 항상 `Current` 문서와 실제 코드 상태를 우선한다.

## 아카이브 위치

- `docs/Archive/2026-04-06/`

아래 자료는 모두 아카이브로 이동했다.

- 기존 코드리뷰 문서
- 실행 프롬프트 문서
- 작업 로그와 변경 기록
- 긴 컴플라이언스 초안
- 공개용 개인정보처리방침 초안 원문
- App Store 개인정보 초안 원문
- 에러 스크린샷 및 보조 자료

## 참고 원칙

- 현재 상태, 우선순위, 출시 판단은 `Current` 문서만 읽어도 파악 가능해야 한다.
- 세부 근거가 필요하면 아카이브를 보되, 아카이브 내용을 그대로 현재 기준으로 간주하지 않는다.
